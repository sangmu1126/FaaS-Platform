...
// Test Options: 50 Virtual Users, 30 seconds duration
export const options = {
    stages: [
        { duration: '5s', target: 20 },  // Ramp up to 20 users
        { duration: '20s', target: 50 }, // Stay at 50 users (Load)
...
export function setup() {
    ...
    // console.log(`[DEBUG] Body: ${res.body}`); // Uncomment if debugging
...
export default function (data) {
    ...
    // B. Function Execution (Heavy)
    const payload = JSON.stringify({
        functionId: data.functionId,
        inputData: { name: 'K6 Cloud Loader' }
    });
    ...
    const checkRes = check(resRun, {
        ...
        // Check for "Processed" which is the output of heavy_task.py
